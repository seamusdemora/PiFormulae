...
*  ### RPi specific 
   ...
   *  [So you want to remove `rpi-eeprom` package & save 50MB?](#want-to-remove-the-rpi-eeprom-package-to-save-25mb-tough-shit-say-the-raspberries) - a new [resolution!](https://github.com/seamusdemora/PiFormulae/blob/master/ForAnyoneWhoOwnsARaspberryPiZero-2-3.md)
...
#### [There's now a "recipe" with steps that resolve this ignorance-induced quandary](https://github.com/seamusdemora/PiFormulae/blob/master/ForAnyoneWhoOwnsARaspberryPiZero-2-3.md) 
...
1. Assuming your ***username is the same*** on your local host and the server, the very simplest (no options required) form of the command is this: 
   ...
   ```bash
    ...
    # e.g. for a server named 'blinky':
    $ ssh-copy-id blinky
   ```

   This will copy your default public key from `~/.ssh/id_ed25519.pub` (for example) to the named server. As long as you have an account on the server, this is all you need. You will be prompted for your password after the `ssh-copy-id` command, but never again - assuming the command was successful of course.

2. Alternatively, you can inspect your `~/.ssh` folder to learn which public key(s) you have; if you have several, you may choose one to use. The current default key generated by `ssh-keygen` (a [useful reference on `ssh-keygen`](https://linuxconfig.org/how-to-generate-and-manage-ssh-keys-on-linux)) is `id_ed25519.pub`, but you may wish to use a *non-default* key (e.g. `id_rsa.pub`). If you need to specify a particular key, use the `-i` option; for example, to specify your `id_rsa.pub` file  :  

      ```bash
       $ ssh-copy-id -i ~/.ssh/id_rsa.pub blinky
      ```


3. Now assume that you are setting up an SSH connection from a host named `myhost`, and your username on `myhost` is `usermy`. Further assume the SSH server you wish to connect to is named `theserver`, and your username on `theserver` is `userthe` . Here's how to use `ssh-copy-id` to transfer your public key from `myhost` to `theserver`: 

   
     ```bash
      $ ssh-copy-id userthe@theserver
     ```
